...
Macbook Duo ships in two flavours from the same sources. On the store it is sold as **Macbook Fold**; `AppBrand.name` picks the name at compile time, and every user-facing string takes it as a `%@` argument.
...
1. Create the app record: platform macOS, name **Macbook Fold**, primary language English, bundle identifier `com.shivamchopra.macbookduo`, any SKU.
